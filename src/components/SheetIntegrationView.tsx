...
function doGet(e) {
  ...
  var sharedConfig = getConfigData(sheetConfig);
...
function doPost(e) {
  ...
  if (action === "save_transaction") {
    ...
    return ContentService.createTextOutput(JSON.stringify({ success: true, message: "Log Notifikasi berhasil dicatat ke Google Sheets!" }))
      .setMimeType(ContentService.MimeType.JSON);
  } else if (action === "sync_config") {
    updateConfig(sheetConfig, postData.config || {}, postData.auth || {}, postData.theme || "light");
    
    return ContentService.createTextOutput(JSON.stringify({ success: true, message: "Profil aplikasi berhasil disimpan ke Google Sheets!" }))
      .setMimeType(ContentService.MimeType.JSON);
    ...
    if (postData.config || postData.auth || postData.theme) {
      updateConfig(sheetConfig, postData.config || {}, postData.auth || {}, postData.theme || "light");
    }
...
function getOrCreateSheet(ss, name) {
  ...
  if (sheet.getLastRow() === 0) {
    ...
    if (name === "Siswa") {
      ...
      sheet.appendRow(["configJson", "authJson", "theme"]);
    }
  }
  return sheet;
}
...
function getConfigData(sheet) {
  ...
  if (values.length <= 1 || !values[1][0]) {
    return { config: {}, auth: {}, theme: "light" };
  }
  
  var config = {};
  var auth = {};
  try { config = JSON.parse(values[1][0] || "{}"); } catch (e) {}
  try { auth = JSON.parse(values[1][1] || "{}"); } catch (e) {}
  return { config: config, auth: auth, theme: values[1][2] || "light" };
}

function updateConfig(sheet, config, auth, theme) {
  sheet.clear();
  sheet.appendRow(["configJson", "authJson", "theme"]);
  sheet.appendRow([JSON.stringify(config || {}), JSON.stringify(auth || {}), theme || "light"]);
}
