...
const safeSetItem = (key: string, value: string) => {
  try {
    ...
    console.warn(`[Storage] Failed to save ${key} to localStorage (quota likely exceeded)`);
  }
};
...
const loadRuntimeConfig = async (): Promise<RuntimeConfig> => {
  const envSheetUrl = import.meta.env.VITE_GOOGLE_SHEET_URL as string | undefined;

  try {
    const response = await fetch("/runtime-config.json?v=" + encodeURIComponent(import.meta.env.VITE_APP_VERSION || "1"));
    if (response.ok) {
      const publicConfig = await response.json();
      return {
        ...publicConfig,
        sheetUrl: envSheetUrl || publicConfig.sheetUrl
      };
    }
  } catch (error) {
    console.warn("[Runtime Config] Public config tidak dapat dimuat.", error);
  }

  return { sheetUrl: envSheetUrl };
};
...
export default function App() {
  ...
  useEffect(() => {
    safeSetItem("KAS_SEKOLAH_THEME", isDark ? "dark" : "light");
  ...
  useEffect(() => {
    ...
    const loadAndSyncBackground = async () => {
      ...
      try {
        ...
        // Vercel/static deployments do not have browser localStorage shared
        // between devices. Bootstrap the public Sheet endpoint from a build
        // variable or runtime-config.json before trying any cloud sync.
        const runtimeConfig = await loadRuntimeConfig();
        const publicAppConfig = runtimeConfig.appConfig || runtimeConfig.config || {};
        const hasCachedConfig = Boolean(cachedConfig);
        if (!hasCachedConfig || !currentConfig.sheetUrl) {
          currentConfig = { ...currentConfig, ...publicAppConfig };
          if (runtimeConfig.sheetUrl) currentConfig.sheetUrl = runtimeConfig.sheetUrl;
          setConfig(currentConfig);
          safeSetItem("KAS_SEKOLAH_CONFIG", JSON.stringify(currentConfig));
          activeSheetUrl = currentConfig.sheetUrl;
        }
        ...
        if (res.ok) {
          ...
          if (body.success && body.data) {
            ...
            if (serverConfig) {
              currentConfig = { ...currentConfig, ...serverConfig };
              setConfig(currentConfig);
              safeSetItem("KAS_SEKOLAH_CONFIG", JSON.stringify(currentConfig));
              activeSheetUrl = currentConfig.sheetUrl;
            }
            ...
            if (auth) {
              if (auth.username) safeSetItem("KAS_SEKOLAH_USER", auth.username);
              if (auth.password) safeSetItem("KAS_SEKOLAH_PASS", auth.password);
            }

            if (theme) {
              setIsDark(theme === "dark");
              safeSetItem("KAS_SEKOLAH_THEME", theme);
            }
          }
        }
      } catch (err) {
        ...
        if (dbRes.ok) {
          ...
          if (dbBody.success && dbBody.data) {
            ...
            if (siswa) {
              ...
              safeSetItem("KAS_SEKOLAH_SISWA", JSON.stringify(normalizedSiswa));
              activeSiswa = normalizedSiswa;
            }
            if (transaksi) {
              setTransaksiList(transaksi);
              safeSetItem("KAS_SEKOLAH_TRANSAKSI", JSON.stringify(transaksi));
              activeTrx = transaksi;
            }
            if (biaya) {
              setBiayaList(biaya);
              safeSetItem("KAS_SEKOLAH_BIAYA", JSON.stringify(biaya));
              activeBiaya = biaya;
            }
            if (logs) {
              setNotificationLogs(logs);
              safeSetItem("KAS_SEKOLAH_LOGS", JSON.stringify(logs));
              activeLogs = logs;
            }
  ...
  const saveLocalDatabaseState = (
    ...
    safeSetItem("KAS_SEKOLAH_SISWA", JSON.stringify(normalizedSiswa));
    safeSetItem("KAS_SEKOLAH_TRANSAKSI", JSON.stringify(newTransaksi));
    
    if (newBiaya) {
      setBiayaList(newBiaya);
      safeSetItem("KAS_SEKOLAH_BIAYA", JSON.stringify(newBiaya));
    }
    if (newLogs) {
      setNotificationLogs(newLogs);
      safeSetItem("KAS_SEKOLAH_LOGS", JSON.stringify(newLogs));
    }
  ...
  const saveLocalTransaksi = (list: Transaksi[]) => {
    ...
    safeSetItem("KAS_SEKOLAH_TRANSAKSI", JSON.stringify(list));
    saveGlobalDatabaseOnServer(undefined, list);
  };

  const saveLocalConfig = (newConfig: AppConfig) => {
    setConfig(newConfig);
    safeSetItem("KAS_SEKOLAH_CONFIG", JSON.stringify(newConfig));
  ...
  const testSheetConnection = async (
    ...
    try {
      ...
      if (body.success && body.data) {
        ...
        // Newer Apps Script deployments also return shared application
        // settings. Keep the Sheet URL from the bootstrap config intact.
        const remoteConfig = body.data.config as Partial<AppConfig> | undefined;
        if (remoteConfig && Object.keys(remoteConfig).length > 0) {
          const mergedConfig = {
            ...config,
            ...remoteConfig,
            sheetUrl: remoteConfig.sheetUrl || url
          };
          setConfig(mergedConfig);
          safeSetItem("KAS_SEKOLAH_CONFIG", JSON.stringify(mergedConfig));
        }
        if (body.data.auth) {
          if (body.data.auth.username) safeSetItem("KAS_SEKOLAH_USER", body.data.auth.username);
          if (body.data.auth.password) safeSetItem("KAS_SEKOLAH_PASS", body.data.auth.password);
        }
        if (body.data.theme) {
          setIsDark(body.data.theme === "dark");
          safeSetItem("KAS_SEKOLAH_THEME", body.data.theme);
        }

        // An entirely empty Sheet must not erase an existing local cache during
        // automatic startup. A manual "Tarik" still passes forceOverwriteEmpty
        // and is allowed to intentionally replace local data with empty lists.
        const hasRemoteRecords = [sheetSiswa, sheetTransaksi, sheetBiaya, sheetLogs]
          .some((items) => items.length > 0);
        if (hasRemoteRecords || forceOverwriteEmpty) {
          setSiswaList(sheetSiswa);
          safeSetItem("KAS_SEKOLAH_SISWA", JSON.stringify(sheetSiswa));
          setTransaksiList(sheetTransaksi);
          safeSetItem("KAS_SEKOLAH_TRANSAKSI", JSON.stringify(sheetTransaksi));
          setBiayaList(sheetBiaya);
          safeSetItem("KAS_SEKOLAH_BIAYA", JSON.stringify(sheetBiaya));
          setNotificationLogs(sheetLogs);
          safeSetItem("KAS_SEKOLAH_LOGS", JSON.stringify(sheetLogs));
        }
  ...
  const syncConfigToSheet = async (
    ...
    if (!targetConfig.sheetUrl) return;

    try {
      const body = await executeSheetsRequest(targetConfig.sheetUrl, "post", {
        action: "sync_config",
        config: targetConfig,
        auth: {
          username: username || localStorage.getItem("KAS_SEKOLAH_USER") || "admin",
          password: password || localStorage.getItem("KAS_SEKOLAH_PASS") || "admin123"
        },
        theme: theme || (isDark ? "dark" : "light")
      });
      if (!body.success) {
        console.warn("[Sheets Config Sync] Target Apps Script belum mendukung sync_config:", body.error);
      }
    } catch (error) {
      console.warn("[Sheets Config Sync] Gagal menyimpan profil bersama:", error);
    }
  };
  ...
  const handleUploadDataToSheet = async (): Promise<{ success: boolean; message: string }> => {
    ...
    try {
      const body = await executeSheetsRequest(config.sheetUrl, "post", {
        ...
        config,
        auth: {
          username: localStorage.getItem("KAS_SEKOLAH_USER") || "admin",
          password: localStorage.getItem("KAS_SEKOLAH_PASS") || "admin123"
        },
  ...
  const saveBiayaMaster = (newList: BiayaSekolah[]) => {
    ...
    safeSetItem("KAS_SEKOLAH_BIAYA", JSON.stringify(newList));
  ...
  const handleAddLog = (newLog: NotifikasiLog) => {
    ...
    safeSetItem("KAS_SEKOLAH_LOGS", JSON.stringify(updated));
  ...
  const handleSaveSchoolSettings = (e: React.FormEvent) => {
    ...
    safeSetItem("KAS_SEKOLAH_USER", finalUser);
    safeSetItem("KAS_SEKOLAH_PASS", finalPass);
    saveGlobalSettings(updated, finalUser, finalPass);
    void syncConfigToSheet(updated, finalUser, finalPass);
      ...
      <header className={`backdrop-blur-md h-16 shrink-0 px-6 flex items-center justify-between z-30 no-print transition-all duration-300 ${isDark ? 'bg-slate-900/40 border-b border-white/10' : 'bg-white border-b border-slate-200 shadow-sm'}`}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className={`text-[10px] font-semibold tracking-wider flex items-center gap-1 transition-colors duration-300 ${isDark ? "text-slate-305" : "text-slate-500"}`}>
